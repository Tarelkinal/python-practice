...
def extension(files):
    ...
    for i in range(len(files)):
        ...
        for j in range(len(file)):
            ...
            ext = file[point_index:len(file)]
        formats.append(ext)
    for i in range(len(formats)):
        if counted_formats.count(formats[i]) == 0:
            temp = formats[i]
            counted_extensions.append(formats[i])
            for f in range(len(formats)):
                if formats[f] == temp:
                    counter = counter + 1
            counted_indexes.append(i)
            counted_formats.append(counter)
            counter = 0
    for i in range(len(counted_formats)):
        if counted_formats[i] == max(counted_formats):
